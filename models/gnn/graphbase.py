...
def label2edge(targets, tensor_shape):
    batch_size, num_sample = targets.size()
    target_node_mask = torch.eq(targets, tensor_shape).type(torch.bool).cuda()
    source_node_mask = ~target_node_mask & ~torch.eq(targets, tensor_shape - 1).type(torch.bool)

    label_i = targets.unsqueeze(-1).repeat(1, 1, num_sample)
    label_j = label_i.transpose(1, 2)

    edge = torch.eq(label_i, label_j).float().cuda()
    target_edge_mask = (torch.eq(label_i, tensor_shape) + torch.eq(label_j, tensor_shape)).type(torch.bool).cuda()
...
def transform_shape(tensor):
        batch_size, tensor_shape, other_dim = tensor.shape
        tensor = tensor.view(1, batch_size * tensor_shape, other_dim)
        return tensor
...
class GraphNetwork(nn.Module):
    def __init__(self, args, cfg):
        ...
        # set size
        self.in_features = cfg.TRAIN.IN_FEATURES
        self.node_features = cfg.TRAIN.NODE_FEATURES
        self.edge_features = cfg.TRAIN.EDGE_FEATURES
        self.num_layers = cfg.TRAIN.BATCH_SIZE #args.num_layers
        self.dropout = 0.2 #args.dropout
        ...
        for l in range(self.num_layers):
            ...
            edge2node_net = NodeUpdateNetwork(
                ...
                num_features=self.node_features if l != self.num_layers - 1 else cfg.TRAIN.BATCH_SIZE - 1, #args.num_class
                dropout=self.dropout if l < self.num_layers - 1 else 0.0)
